    ...
    try
        if ~isadir{i}
            ...
            %Read in coordinates - assumes x,y
                ...
                % Compute bound cones mask
                dt = delaunayTriangulation(coords);
                [~, R] = voronoiDiagram(dt);
                
                isBound = true(size(coords,1),1);
                for ii = 1:length(R)
                    if any(R{ii} == 1)
                        isBound(ii) = false;
                    end
                ...
                boundCoords = coords(isBound, :);
                
                if size(boundCoords,1) > maxBound
                    warning(['More than ', num2str(maxBound), ' bound cones detected. Limiting analysis to first ', num2str(maxBound), ' bound cones.']);
                    boundCoords = boundCoords(1:maxBound, :);
                end
                
                % Use only boundCoords for analysis:
                coords = boundCoords;

                
                % It should ONLY be a coordinate list, that means x,y, and nothing else.
                if size(coords,2) ~= 2
