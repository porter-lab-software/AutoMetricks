...
%Modified to automatically shirnk the bounds to get the number of bound
%cells between 95 and 105 by Godfred Sakyi-Badu

max_iter = 20; %Sets the upper limit for iterations. After 20 iterations if it cannot get within the range then the ROI size is probably too small 

% Define margin in pixels to shrink bounds (adjust this number as needed)
margin_px=5; %Initialize the margin pixel 
...
NumBoundCells = sum(bound); %Gets the total number of Bound Cells

if NumBoundCells>=95 && NumBoundCells<=105
    
    break %Break out of the loop if we find the number of bound cells needed
end

if NumBoundCells <95
    margin_px = margin_px -1;% if NumBoundCells is too small, we shrink the margin 
    continue
elseif NumBoundCells>105
    margin_px = margin_px +1;%else we expand the margin
    continue
end
